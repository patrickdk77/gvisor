...
func RegisterFlags(flagSet *flag.FlagSet) {
	...
	flagSet.Duration("revalidate-ttl", 0, "if non-zero, cached file metadata on mounts shared with other users of the host filesystem is trusted for this duration before being revalidated. This is the sentry's equivalent of NFS's actimeo: it bounds staleness rather than eliminating it, in exchange for not consulting the host on every access. Zero (default) revalidates on every access.")
	...
	flagSet.String("proc-hidepid", "", "sets hidepid= on the container's /proc mount: 1 (or noaccess) keeps other users' process directories listed but inaccessible, 2 (or invisible) hides them. This is what normally limits ps to a user's own processes; leave empty to mount /proc as the runtime specifies.")
	flagSet.String("apparmor-policy-source", "none", "where to read AppArmor profiles from in order to derive in-sandbox confinement: none, host, or container. The sentry enforces the parts of a profile the host kernel cannot mediate for a sandboxed application, notably 'owner' rules; 'container' also works on hosts without AppArmor, at the cost of letting the workload supply its own policy.")
	flagSet.String("apparmor-policy-dir", "/etc/apparmor.d", "directory to read AppArmor profiles from, in the host's or the container's filesystem according to --apparmor-policy-source.")
	flagSet.String("apparmor-audit-target", "stderr", "where in-sandbox AppArmor audit records are written: stderr (default) or stdout for the container's own streams, which is where a cluster's log collection reads them from, gvisor for the sentry log, or none to discard them.")
