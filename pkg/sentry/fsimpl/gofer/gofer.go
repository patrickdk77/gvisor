...
type inode struct {
	...
	//	- If this i represents a regular file or directory, readFD (if not
...
// incRef increments the reference count on the inode. The ref count is used to determine
// whether the inode can be destroyed and removed from the inode cache.
func (i *inode) incRef() {
	i.refs.IncRef()
}
...
func (i *inode) destroy(ctx context.Context, d *dentry) {
	i.handleMu.Lock()
...
type dentry struct {
	...
	// deleted. It is accessed using atomic memory operations.
...
func (d *dentry) setStat(ctx context.Context, creds *auth.Credentials, opts *vfs.SetStatOptions, mnt *vfs.Mount) error {
	...
	// To avoid making the SetStat RPC if UID/GID is not actually changing.
...
func (d *dentry) destroyDisconnected(ctx context.Context, destroyInode bool) {
	if destroyInode {
		d.inode.destroy(ctx, d)
	}
	if !d.inode.isSynthetic() {
